...
/** @type {Svelters_Shared_Web_Api_Calorie_Log_Get} */
const endpoint = await container.get('Svelters_Shared_Web_Api_Calorie_Log_Get$');
const RESULT = endpoint.getResultCodes();

describe('Calorie_Log_Get', function () {
    ...
    it('should retrieve data', async function () {
        ...
        const payload = endpoint.createReq();
        payload.dateFrom = '2025-01-01';
        payload.dateTo = '2999-12-31';
        const {statusCode, body} = await helpWeb.post({
            hostname: HOST,
            path: '/app/api/calorie/log/get',
            headers,
            payload,
            timeout: 100000,
        });
        assert.strictEqual(statusCode, 200, 'Unexpected HTTP status code received.');
        /** @type {Svelters_Shared_Web_Api_Calorie_Log_Get.Response} */
